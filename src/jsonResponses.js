...
const deleteCard = (request, response) => {
  ...
  const result = database.deleteCard(number);
...
const postCard = (request, response) => {
  ...
  if (params.type === 'Monster') {
    ...
    } else if (newCard.Types.includes('Xyz')) {
      newCard.Rank = 1;
      newCard.Level = '';
    } else {
      newCard.Level = 1;
    ...
    if (params.ATK) {
      atkDef[0] = parseInt(params.ATK, 10);
      if (atkDef[0] < 0) return badRequest(request, response, statsErr);
    }
    if (params.DEF) {
      atkDef[1] = parseInt(params.DEF, 10);
    ...
    if (newCard.Types.includes('Link')) {
      ...
      atkDef[1] = parseInt(params.link, 10);
      newCard.ATK_DEF = '';
    ...
    if (params.scale) {
      newCard.Pendulum_Scale = parseInt(params.scale, 10);
...
const byRange = (request, response) => {
  ...
  const minDEF = parseInt(request.query.minDEF, 10);
  const maxDEF = parseInt(request.query.maxDEF, 10);
